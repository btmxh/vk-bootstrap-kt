        ...
        stack {
            if (!instance.headless && !deferSurfaceInitialization) {
                if (surface == VK_NULL_HANDLE) {
                    io.github.ngoduyanh.vkb.PhysicalDeviceError.NO_SURFACE_PROVIDED.throwError()
                }
            }
            val ip = mallocInt(1)
            ...
            if (ret != VK_SUCCESS) {
                io.github.ngoduyanh.vkb.PhysicalDeviceError.FAILED_ENUMERATE_PHYSICAL_DEVICES.throwError(ret)
            }
            if (!physicalDevices.hasRemaining()) {
                io.github.ngoduyanh.vkb.PhysicalDeviceError.NO_PHYSICAL_DEVICES_FOUND.throwError()
            }
            ...
            selectedDevice ?: io.github.ngoduyanh.vkb.PhysicalDeviceError.NO_SUITABLE_DEVICE.throwError()
