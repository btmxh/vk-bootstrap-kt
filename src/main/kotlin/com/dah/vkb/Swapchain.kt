...
data class Swapchain(
    ...
    inline fun getImages(allocator: (Int) -> LongBuffer = ::memAllocLong) = runCatching {
        stack {
            ...
            if (ret != VK_SUCCESS) {
                SwapchainError.FAILED_GET_SWAPCHAIN_IMAGES.throwError(ret)
            }
            images
        }
    }
    ...
    fun build() = runCatching {
        stack {
            if (surface == VK_NULL_HANDLE) {
                SwapchainError.SURFACE_HANDLE_NOT_PROVIDED.throwError()
            }
            ...
            if (ret != VK_SUCCESS) {
                SwapchainError.FAILED_CREATE_SWAPCHAIN.throwError(ret)
            }
