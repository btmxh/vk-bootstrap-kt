        ...
        stack {
            ...
            if (requiredAPIVersion > vk10 || desiredAPIVersion > vk10) {
                ...
                if (queriedAPIVersion < requiredAPIVersion) {
                    when (VK_VERSION_MINOR(requiredAPIVersion)) {
                        ...
                        2 -> InstanceError.VULKAN_VERSION_1_2_UNAVAILABLE.throwError()
                        0 -> InstanceError.VULKAN_UNAVAILABLE.throwError()
                        else -> InstanceError.VULKAN_VERSION_1_1_UNAVAILABLE.throwError()
                    }
                }
            ...
            if (!headlessContext) {
                ...
                if (!khrSurfaceAdded || !addWindowEXTs) {
                    InstanceError.WINDOWING_EXTENSIONS_NOT_PRESENT.throwError()
                }
            }

            extensions.flip()
            if (!checkExtensionsSupported(systemInfo.availableExtensions, extensions)) {
                InstanceError.REQUESTED_EXTENSIONS_NOT_PRESENT.throwError()
            }
            ...
            if (!checkLayersSupported(systemInfo.availableLayers, layers)) {
                InstanceError.REQUESTED_LAYERS_NOT_PRESENT.throwError()
            }
            ...
            if (ret != VK_SUCCESS) {
                InstanceError.FAILED_CREATE_INSTANCE.throwError(ret)
            }
...
//            debugCallback?.free()
            ...
            val debugMessenger = if (useDebugMessenger) {
                ...
                if (ret != VK_SUCCESS) {
                    InstanceError.FAILED_CREATE_DEBUG_MESSENGER.throwError(ret)
                }
                pMessenger[0]
            } else VK_NULL_HANDLE
